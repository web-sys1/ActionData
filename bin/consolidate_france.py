...
for typ, key in [("deces", "deces"), ("hospitalisations", "hospitalises"), ("soins_critiques", "reanimation"), ("retour_a_domicile", "gueris"), ("cas_positifs", "cas_confirmes"), ("vaccins_premiere_dose", "vaccines_premiere_dose"), ("vaccins_vaccines", "vaccines_entierement")]:
    with open(os.path.join("covid19/data_sources", "france-%s.json" % typ)) as f:
        typdata = json.load(f)
...
with open(os.path.join("covid19/data_sources", "chiffres-cles.csv"), "a") as f:
    f.write("\n")
    writer = csv.writer(f)
...
with open(os.path.join("covid19/data_sources", "france.csv"), "w") as f:
    writer = csv.writer(f)
